...
DF.Carseats <- subset(Carseats, select = c(Sales, Price, Advertising, ShelveLoc))
...
Размерность обучающей выборки: $n = 400$ строк, $p = 2$ объясняющих переменных. Зависимая переменная – *Sales*. Дискретная величина - *ShelveLoc*.
...
table(DF.Carseats$ShelveLoc)

```


Построим графики разброса, показав факторы *ShelveLoc* (число цилиндров)  цветом. Зависимой переменной модели является *Sales*, её покажем в первой строке / столбце матричного графика. Во вторую строку / столбец поставим фактор.

Для наглядности вновь разбиваем график на несколько.


```{r}

# переводим дискретные количественные переменные в факторы
DF.Carseats$ShelveLoc <- as.factor(DF.Carseats$ShelveLoc)

# графики разброса, цвет -- количество цилиндров
ggpairs(DF.Carseats[, c(1, 2, 4)], ggplot2::aes(color = ShelveLoc))

ggpairs(DF.Carseats[, c(1, 3, 4)], ggplot2::aes(color = ShelveLoc))

```


#### Графики зависимости  Sales ~ Price ,  Sales ~ Advertising, Sales ~ ShelveLoc
```{r}
...
plot(DF.Carseats$Advertising, DF.Carseats$Sales,
     xlab = 'Advertising', ylab = 'Sales', pch = 21,
     col = rgb(0, 0, 1, alpha = 0.4), bg = rgb(0, 0, 1, alpha = 0.4))

plot(DF.Carseats$ShelveLoc, DF.Carseats$Sales,
     xlab = 'ShelveLoc', ylab = 'Sales', pch = 21,
     col = rgb(0, 0, 1, alpha = 0.4), bg = rgb(0, 0, 1, alpha = 0.4))
...
ggplot(
  DF.Carseats, aes(x = ShelveLoc, y = Sales)) + 
...
$$Sales=f(Price + Advertising + ShelveLoc)$$


Линейная модель: 

$$Sales=β_0+β_1⋅weihgt +β_2  Advertising + β_3 ShelveLoc$$
...
fit.lm.1_fic <- lm(Sales ~ Price + Advertising + ShelveLoc, subset = inTrain)

# подгонка линейной модели на обучающей выборке
fit.lm.1_fic <- lm(Sales ~ Price + Advertising + ShelveLoc, 
               subset = inTrain)

# прогноз на тестовую
y.test.lm.1_fic <- predict(fit.lm.1_fic, DF.Carseats[-inTrain, ])

# считаем MSE на тестовой выборке
MSE.lm.1_fic <- mean((y.test.fact - y.test.lm.1_fic)^2)
...
$$Sales = β_0 + β_1Price  + β_2 Advertising + β_3ShelveLoc + β_4 Price ^2 + β_5 Advertising^2 + β_6 ShelveLoc^2$$
...
# подгонка модели на обучающей выборке
fit.lm.2_fic <- lm(Sales ~ poly(Price , 2)  + poly(Advertising, 2) + ShelveLoc, subset = inTrain)

# прогноз на тестовую
y.test.lm.2_fic <- predict(fit.lm.2_fic, DF.Carseats[-inTrain, ])

# считаем MSE на тестовой выборке
MSE.lm.2_fic <- round(mean((y.test.fact - y.test.lm.2_fic)^2), 2)
...
$$Sales=β_0+β_1Price + β_1Price  + β_2 Advertising +  β_3 ShelveLoc  

+ β_4 Advertising^2+ + β_5Price ^2 + β_6 ShelveLoc^2 + β_7⋅Price ^3 + β_8 Advertising^3 + β_9 ShelveLoc^3$$
...
# подгонка модели на обучающей выборке
fit.lm.3_fic <- lm(Sales ~ poly(Price , 3) + poly(Advertising, 3) + ShelveLoc, 
               subset = inTrain)

# прогноз на тестовую
y.test.lm.3_fic <- predict(fit.lm.3_fic, DF.Carseats[-inTrain, ])

# считаем MSE на тестовой выборке
MSE.lm.3_fic <- round(mean((y.test.fact - y.test.lm.3_fic)^2), 2)
...
# подгонка линейной модели на обучающей выборке
fit.glm_fic <- glm(Sales ~ Price + Advertising + ShelveLoc, data = DF.Carseats)

# считаем LOOCV-ошибку
cv.err_fic <- cv.glm(DF.Carseats, fit.glm_fic)
...
cv.err.loocv_fic <- rep(0, 5)
# имена элементов вектора
names(cv.err.loocv_fic) <- 1:5

# цикл по степеням полиномов
for (i in 1:5) {
  # оценка модели
  fit.glm_fic <- glm(Sales ~ poly(Price , i) + poly(Advertising, i) + ShelveLoc, data = DF.Carseats)
  # расчёт ошибки
  cv.err.loocv_fic[i] <- cv.glm(DF.Carseats, fit.glm_fic)$delta[1]
}
...
$$Sales=f(Price + Advertising)$$


Линейная модель: 

$$Sales=β_0+β_1⋅weihgt +β_2 Advertising$$
...
# подгонка модели на обучающей выборке
fit.lm.1 <- lm(Sales ~ Price  + Advertising, subset = inTrain)

# подгонка линейной модели на обучающей выборке
fit.lm.1 <- lm(Sales ~ Price  + Advertising, 
               subset = inTrain)
# прогноз на тестовую
...
$$Sales = β_0 + β_1Price  + β_2 Advertising + β_3Price ^2 + β_4Advertising^2$$
...
# подгонка модели на обучающей выборке
fit.lm.2 <- lm(Sales ~ poly(Price , 2) + poly(Advertising, 2), subset = inTrain)
...
$$Sales=β_0 + β_1Price + β_2 Advertising  + 

+ β_3 Advertising^2 + + β_4Price ^2 + β_5⋅Price ^3 + β_6 Advertising^3$$
...
# подгонка модели на обучающей выборке
fit.lm.3 <- lm(Sales ~ poly(Price , 3) + poly(Advertising, 3), 
               subset = inTrain)
...
for (i in 1:5) {
  # оценка модели
  fit.glm <- glm(Sales ~ poly(Price , i) + poly(Advertising, i), data = DF.Carseats)
  # расчёт ошибки
...
for (i in 1:5) {
  # оценка модели
  fit.glm_1 <- glm(Sales ~ poly(Price , i) + poly(Advertising, i) + ShelveLoc, data = DF.Carseats)
  # расчёт ошибки
...
df.MSE <- data.frame(Модель = c('Линейная', 'Полином 2 степени',
                     ...
                     MSE = round(c(MSE.lm.1_fic, MSE.lm.2_fic, MSE.lm.3_fic, 
                                  cv.err.loocv_fic, cv.err.k.fold10_1, cv.err.k.fold5_1), 2))
...
boot.fn.2 <- function(data, index){
  coef(lm(Sales ~ poly(Price , 2)  +  poly(Advertising, 2), data = data, subset = index))
}
